...
namespace LightConquer_Project.Panels
{
    public partial class ChatPanal : Form
    {
        ...
        private void Form1_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                LightConquer_Project.Client.GameClient client;
        ...
        private void Send_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (var user in Database.Server.GamePoll.Values)
                {
                    ...
                    {
                        user.SendWhisper(SendText.Text, "CloudStrife[PM]", SelectedClient);
                        Clients[SelectedClient].Mess.Add("[" + DateTime.Now.ToString() + "] CloudStrife[PM]>>>" + SendText.Text);
        ...
        private void ChatPanal_Load(object sender, EventArgs e)
        {
            ...
            {
                LightConquer_Project.Client.GameClient pclient = new LightConquer_Project.Client.GameClient(null);
                pclient.Fake = true;
                ...
                pclient.Player.Name = "CloudStrife[PM]";
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                foreach (var user in Database.Server.GamePoll.Values)
                {
                    if (user.Player.Name == SelectedClient)
                    {
                        user.SendWhisper(comboBox1.Text, "CloudStrife[PM]", SelectedClient);
                        Clients[SelectedClient].Mess.Add("[" + DateTime.Now.ToString() + "] CloudStrife[PM]>>>" + comboBox1.Text);
                        var x = Clients[SelectedClient].Mess.Last();
                        RecList.Items.Add(x);
                        comboBox1.Text = "";
                    }
