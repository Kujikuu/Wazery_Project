    ...
    public unsafe class SobNpc : IMapObj
    {
        ...
        public unsafe void Die(ServerSockets.Packet stream, Client.GameClient killer)
        {
            ...
            if (UID >= 7002 && UID <= 7003)
            {
                ...
                //{
                //    int val = Program.GetRandom.Next(1, 10);
                //    killer.Player.Nobility.Donation += (uint)(val);
                //    killer.Send(stream.NobilityIconCreate(killer.Player.Nobility));
                //    Program.NobilityRanking.UpdateRank(killer.Player.Nobility);
                //    //client.Player.Nobility.Donation
                //    killer.SendSysMesage($"You got " + val + " Nobility Points !");
                //}
                return;

            }
            if (killer.EventBase != null)
            {
                ...
                if (HitPoints == 0)
                {
                    if (killer.EventBase.Stage == Game.MsgEvents.EventStage.Fighting)
                        killer.EventBase.Kill(killer, this);
                }
                else
                {
                    if (killer.EventBase.Stage == Game.MsgEvents.EventStage.Fighting)
                        killer.EventBase.Hit(killer, this, Damage);
                }
            }
            if (IsStatue)
            {
