...
namespace LightConquer_Project.Role.Instance
{
    public class Equip
    {
            ...
            get
            {
                ...
                foreach (var item in CurentEquip)
                {
                    if (item.Position != (ushort)Role.Flags.ConquerItem.Steed
                        ...
                        && item.Position != (ushort)Role.Flags.ConquerItem.Wing
        ...
        public int AttackSpeed(bool physical)//bahaa
        {
            int speed = 800;
        ...
        public bool UseEpicTaoWeapon()
        {
            foreach (var item in CurentEquip)
            {
                if (item != null)
                {
                    if (Database.ItemType.IsTaoistEpicWeapon(item.ITEM_ID))
                        return true;
                }
            }
            return false;
        }
        public bool UseEpicMonkWeapon()
        {
            foreach (var item in CurentEquip)
            {
                if (item != null)
        ...
        public unsafe void UpdateStats(Game.MsgServer.MsgGameItem[] MyGear, ServerSockets.Packet stream)
        {
            ...
            try
            {
                ...
                Owner.Status.MaxAttack = (ushort)(Owner.Player.Strength);
                Owner.Status.MinAttack = (ushort)(Owner.Player.Strength);
                //       Owner.Status.MagicDamage = Owner.Player.Spirit;// (ushort)(Owner.Player.Spirit * 10);
                ...
                foreach (var item in MyGear)
                {
                    ...
                    {
                        if (Owner.Equipment.TryGetEquip(Role.Flags.ConquerItem.LeftWeapon, out shiled))
                        {
                            if (Database.ItemType.IsShield(shiled.ITEM_ID))
                            {
                        ...
                        {
                            Owner.PrestigeLevel += (item.PerfectionLevel * 2);
                        }
                    }
                    else
                    {
                    ...
                    try
                    {
                        ...
                        if (item.Purification.InLife)
                        {
                            ...
                            if (ItemPostion == (ushort)Role.Flags.ConquerItem.LeftWeapon)
                            {
                                Owner.Status.MaxAttack += (uint)(purificare.MaxAttack / 2);
                                Owner.Status.MinAttack += (uint)(purificare.MinAttack / 2);
                            }
                            else
                            {
                            ...
                            if (extraitematributes != null)
                            {
                                ...
                                else
                                {
                                    if (ItemPostion == (ushort)Role.Flags.ConquerItem.LeftWeapon)// just for main attack
                                    {
                                        ...
                                        Owner.Status.MagicAttack += (uint)(extraitematributes.MagicAttack / 2);
                                    }
                                    else
                                    {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Console.WriteLine("ID == " + item.ITEM_ID + ".");
                    }
                }
                ...
                //Owner.Status.MagicAttack += (uint)(Owner.Status.MagicAttack * Owner.GemValues(Role.Flags.Gem.NormalPhoenixGem)) / 123;
                Owner.Status.ItemBless += Owner.GemValues(Role.Flags.Gem.NormalTortoiseGem) / 3;
                ...
                Database.PerfectionPower.CreateStatusAtributes(Owner);
                ...
                if (Database.ItemType.IsTrojanEpicWeapon(LeftWeapon) || Database.ItemType.IsTrojanEpicWeapon(RightWeapon))
                {
                    if (!Owner.MySpells.ClientSpells.ContainsKey((ushort)Role.Flags.SpellID.BreathFocus))
                    {
                        Owner.MySpells.Add(stream, (ushort)Role.Flags.SpellID.BreathFocus);
                    }
                }
            }
