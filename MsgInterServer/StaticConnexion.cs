...
namespace LightConquer_Project.MsgInterServer
{
       ...
       public static void ProcesReceive(ServerSockets.SecuritySocket obj, ServerSockets.Packet stream)
       {
           ...
           try
           {
               switch (PacketID)
               {
                   ...
                   case LightConquer_Project.Game.GamePackets.Chat:
                       {
                           ...
                           if (mes.ChatType == MsgMessage.ChatMode.BroadcastMessage)
                           {
                               if (mes.__Message == "[Cross Elite PK Tournament] begins at 20:00. Get yourself prepared for it!")
                               {
                                   ...
                                   LightConquer_Project.Game.MsgTournaments.MsgSchedules.SendInvitation("Cross Elite PK Tournament", "[Special Accesory/Boots,Cps and more rewards]", 293, 160, 1002, 0, 60);
                                   break;
                               }
                           }
