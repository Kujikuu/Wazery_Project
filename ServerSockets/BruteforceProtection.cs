        ...
        private void _internalInit()
        {
            ...
            while (true)
            {
                ...
                foreach (BruteForceEntry bfe in collection.GetValues())
                {
                    ...
                    {
                        if (bfe.Unbantime.Value <= Now.Value) { collection.Remove(bfe.IPAddress); }
                    }
                }
                Thread.Sleep(2000);
            }
        }
        public void Init(int WatchBeforeBan) { BanOnWatch = WatchBeforeBan; new Thread(new ThreadStart(_internalInit)).Start(); }
        public void AddWatch(string IPAddress)
        {
            lock (collection)
            {
                BruteForceEntry bfe; if (!collection.TryGetValue(IPAddress, out bfe)) { bfe = new BruteForceEntry(); bfe.IPAddress = IPAddress; bfe.WatchCheck = 1; bfe.AddedTimeRemove = Extensions.Time32.Now.AddMinutes(3); bfe.Unbantime = new Extensions.Time32(0); collection.Add(IPAddress, bfe); }
                else
                {
                    bfe.WatchCheck++; if (bfe.WatchCheck >= BanOnWatch)
                    {
