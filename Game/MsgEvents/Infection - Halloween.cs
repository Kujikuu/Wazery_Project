    ...
    public class HalloweenInfection : Events
    {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                foreach (GameClient c in PlayerList.Values)
                {
                    ...
                    if (TeamOne.Count <= TeamTwo.Count)
                    {
                        ...
                        c.Player.AddSpecialGarment(stream, 193445);
                    ...
                    else
                    {
                        ...
                        c.Player.AddSpecialGarment(stream, 193525);
                ...
                Teams.Add(193445, TeamOne);
                Teams.Add(193525, TeamTwo);
            }
        ...
        public override void Hit(GameClient Attacker, GameClient Victim)
        {
            if (Stage == EventStage.Fighting)
            {
                ...
                {
                    var stream = rec.GetStream();
                    if (Teams[193445].ContainsKey(Attacker.EntityID))
                    {
                        if (Teams[193525].ContainsKey(Victim.EntityID))
                            Teams[193525].Remove(Victim.EntityID);

                        Teams[193445].Add(Victim.EntityID, Victim);
                        Victim.SendSysMesage($"You've joined the Glaze!");

                        //Victim.MyClient.AddSend(Packets.OverwriteGarment(TeamOneGarment));
                        Victim.Player.AddSpecialGarment(stream, 193445);

                    }
                    else if (Teams[193525].ContainsKey(Attacker.EntityID))
                    {
                        if (Teams[193445].ContainsKey(Victim.EntityID))
                            Teams[193445].Remove(Victim.EntityID);

                        Teams[193525].Add(Victim.EntityID, Victim);
                        Victim.SendSysMesage($"You've joined the Glory!");
                        Victim.Player.AddSpecialGarment(stream, 193525);
