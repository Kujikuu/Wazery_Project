...
{
    public class EliteLadderTournament : Events
    {
        bool start = false;
        bool nextRound = false;
        public EliteLadderTournament()
        {
            EventTitle = "Elite Ladder Tournament";
            Duration = 20;
            BaseMap = 1507;
            NoDamage = true;
            MagicAllowed = false;
        ...
        public override void Hit(GameClient Attacker, GameClient Victim)
        {
            ...
            {
                if (PlayerScores[Victim.EntityID] < 48)
                {
                    PlayerScores[Victim.EntityID]++;
                    Victim.SendSysMesage($"You can only be hitted " + (50 - PlayerScores[Victim.EntityID]) + " more times!", MsgServer.MsgMessage.ChatMode.FirstRightCorner);

                }
                else if (PlayerScores[Victim.EntityID] == 48)
                {
                    PlayerScores[Victim.EntityID]++;
        ...
        public override void DisplayScore()
        {
            ...
            foreach (KeyValuePair<uint, List<uint>> M in mapsPairs.ToList())
            {
                if (M.Value.Count == 2)
                {
                    Broadcast($"{PlayerList[M.Value[0]].Name} - {(50 - PlayerScores[M.Value[0]])} VS {PlayerList[M.Value[1]].Name} - {(50 - PlayerScores[M.Value[1]])}", BroadCastLoc.Score, Score);
                    Score++;
                }
            }
