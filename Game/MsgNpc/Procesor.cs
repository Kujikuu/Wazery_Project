            ...
            protected unsafe override void OnDequeue(InvokerClient action, int time)
            {
                try
                {
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        ...
                        if (action.npcid == (uint)NpcID.SelectSacredRefineryPack 
                            || action.npcid == (uint)NpcID.SelectP7WeaponSoulPack
                            ...
                            || action.npcid == (uint)NpcID.BlackFridayMountPack 
                            || action.npcid == (uint)NpcID.BlackFridayAccesory
                            || action.npcid == (uint)NpcID.EventsRawrderToken
                           ...
                           || action.npcid == (uint)NpcID.MrMirror2
                            ...
                            || action.npcid == (uint)NpcID.Joyful5StonePack
                             //
                             || action.npcid == (uint)NpcID.VotePointsReward
                              || action.npcid == (uint)NpcID.Invitor
                            //
                            || action.npcid == (uint)NpcID.DailyEventQuest
                            || action.npcid == (uint)NpcID.EpicPack
                            || action.npcid == (uint)NpcID.JiangEpic
                            || action.npcid == (uint)NpcID.CHIPOWERSELECTOR
                            || action.npcid == (uint)NpcID.BigHunt1
                            || action.npcid == (uint)NpcID.BigHunt2
                            || action.npcid == (uint)NpcID.BigHunt3
                            || action.npcid == (uint)NpcID.SuperGemBox
