...
using LightConquer_Project.MsgInterServer.Packets;
using Extensions;
namespace LightConquer_Project.Game.MsgServer
{
    using static MsgMessage;
    ...
    public unsafe static class MsgDataPacket
    {
        ...
        public unsafe static void Bulletin(Client.GameClient client, ServerSockets.Packet msg, ActionQuery* data)
        {
          ...
          || client.Player.Map == 1505 || client.Player.Map == 1506 || client.Player.Map == 1509 || client.Player.Map == 1508 || client.Player.Map == 1507)
            ...
            if (client.Player.Map == 1038 || client.Player.Map == MsgTournaments.MsgClassPKWar.MapID || client.Player.Map == MsgTournaments.MsgWeeklyPKChampion.MapID || client.Player.Map == MsgTournaments.MsgMonthlyPKChampion.MapID || client.Player.DynamicID != 0
                || client.Player.Map == 6001 || client.Player.Map == 6003)
            {
        ...
        [DataAttribute(ActionType.CreditGifts)]//bahaa
        public unsafe static void CreditGifts(Client.GameClient client, ServerSockets.Packet msg, ActionQuery* data)
        {
            if ((client.Player.MainFlag & Role.Player.MainFlagType.CanClaim) ==  Role.Player.MainFlagType.CanClaim)
            {
                ...
                {
                    if (client.Inventory.HaveSpace(19))
                    {
                        client.Inventory.Add(msg, 723794, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, false);//PowerExpPack
                        client.Inventory.Add(msg, 3300035, 10, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, false);//ExchangeToken
                        client.Inventory.Add(msg, 3100053, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, false);//AccessPack
                        client.Inventory.Add(msg, 3100055, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, false);//GarmentPack
                        client.Inventory.Add(msg, 3100054, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, false);//MountArmorPack
                        client.Inventory.Add(msg, 3001289, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, true);//PromotionGift
                        client.Inventory.Add(msg, 3001035, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, true);//ChiPointPack 1000
                        client.Inventory.Add(msg, 730008, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, false);//+8Stone
                        client.Inventory.Add(msg, 780001, 1, 0, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, true);//VIP[3-Days]
                ...
                if (!client.Player.Alive)
                {
                    ...
                    if (ReviveHere)
                    {
                        if (client.Player.Map == 1038 || client.Player.Map == MsgTournaments.MsgClassPKWar.MapID
                            || client.Player.Map == MsgTournaments.MsgWeeklyPKChampion.MapID
                            || client.Player.Map == MsgTournaments.MsgMonthlyPKChampion.MapID
        ...
        public unsafe static void UpdateSpell(Client.GameClient client, ServerSockets.Packet msg, ActionQuery* data)//bahaa
        {
            //client.SendSysMesage("Sorry, you can't upgrade spell using ConquerPoints.");
            MsgSpell Spell;
        ...
        public unsafe static void UpdateProf(Client.GameClient client, ServerSockets.Packet msg, ActionQuery* data)
        {
            ...
            if (client.MyProfs.ClientProf.TryGetValue(data->dwParam, out prof))
            {
                if (prof.Level < 20)
                {
                    ushort PriceUpdate = Database.Server.PriceUpdatePorf2[prof.Level];
...
//#endif
        ...
        public unsafe static void CompleteLogin(Client.GameClient client, ServerSockets.Packet msg, ActionQuery* data)
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                {//Welcome back, %user_name. We hope you're having a great day. Enjoy the server!
                    Program.SendGlobalPackets.Enqueue(new MsgMessage("Player, " + client.Player.Name + ". has joined the game!", MsgColor.white, ChatMode.TopLeft).GetArray(stream));
                }
            }
