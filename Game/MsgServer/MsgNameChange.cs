                       ...
                       case ActionID.Request:
                           {
                               using (var rec = new ServerSockets.RecycledPacket())
                               {
                                   ...
                                   if (user.Player.ConquerPoints < 20000)
                                       break;
                                   if (Name != null && Name != "" && Name.Length >= 2 && Program.NameStrCheck(Name) && user.Player.NameEditCount < 3 && !Database.Server.NameUsed.Contains<int>(Name.GetHashCode()))
                                   {
                                       user.Player.NameEditCount += 1;
                                       user.Send(stream.NameChangeCreate(ActionID.Success, user.Player.NameEditCount, 3, Name));
