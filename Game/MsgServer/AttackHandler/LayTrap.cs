                    ...
                    case (ushort)Role.Flags.SpellID.TwilightDance:
                        {
                            ...
                            foreach (var coords in Line.LCoords)
                            {
                                ...
                                var FloorItem = new Role.FloorSpell(40, (ushort)coords.X, (ushort)coords.Y, Color, DBSpell, Stamp)
                                {
                                    FloorPacket =
                                    {
                                        OwnerX = user.Player.X,
                                        OwnerY = user.Player.Y,
                                        ItemOwnerUID = user.Player.UID
                                    }
                                };
                                user.Player.FloorSpells[ClientSpell.ID].AddItem(FloorItem);
                                //Color++;
