...
namespace LightConquer_Project.Game.MsgServer.AttackHandler
{
    public class AttachStatus
    {
                    ...
                    case (ushort)Role.Flags.SpellID.SpiritFocus:
                        {
                            ...
                            user.Player.SpiritFocusStamp = Extensions.Time32.Now;
                            user.Player.InUseSpiritFocus = true;
                            user.Player.SpiritFocusDamage = (int)DBSpell.Damage;
       ...
       //                     MsgSpell.Send(user);
                    ...
                    case (ushort)Role.Flags.SpellID.Poison:
                        {
                            MsgSpellAnimation MsgSpell = new MsgSpellAnimation(user.Player.UID
                            , 0, Attack.X, Attack.Y, ClientSpell.ID
                            , ClientSpell.Level, ClientSpell.UseSpellSoul);
                            ...
                            if (user.Player.UID == Attack.OpponentUID)
                            {
                                user.Player.AddSpellFlag(MsgUpdate.Flags.Poisoned, (int)DBSpell.Duration, true);
                                MsgSpell.Targets.Enqueue(new MsgSpellAnimation.SpellObj(user.Player.UID, 0, MsgAttackPacket.AttackEffect.None));
                            }
                            else
                            {
                                ...
                                if (user.Player.View.TryGetValue(Attack.OpponentUID, out target, Role.MapObjectType.Monster))
                                {
                                    ...
                                    Damage = (uint)Calculate.Base.MulDiv((int)attacked.HitPoints, (int)(100 - DBSpell.Damage), 100);
                                    attacked.AddSpellFlag(MsgUpdate.Flags.Poisoned, (int)DBSpell.Duration, true);
                                    MsgSpell.Targets.Enqueue(new MsgSpellAnimation.SpellObj(attacked.UID, 0, MsgAttackPacket.AttackEffect.None));
                                }
                                else if (user.Player.View.TryGetValue(Attack.OpponentUID, out target, Role.MapObjectType.Player))
                    ...
                    case (ushort)Role.Flags.SpellID.XpFly:
                        {
                            ...
                            if (user.Player.OnTransform || user.Player.ContainFlag(MsgUpdate.Flags.Ride) || user.Player.ContainFlag(MsgUpdate.Flags.PathOfShadow) || user.Player.ContainFlag(MsgUpdate.Flags.Shield))
                            {
                                user.SendSysMesage("You can't use this skill right now!");
                                break;
                            }
