...
namespace LightConquer_Project.Game.MsgTournaments
{
...
#else 
                    MsgSchedules.SendInvitation("" + Type.ToString() + "City ClanWar", "Silvers and SuperCity Pack", RequestX, RequestY, RequestMap, 0, 60);
#endif
                }
            }
 ...
 Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the ClanWar with a score of " + Winner.Score.ToString() + ""
                ...
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the ClanWar with a score of " + Winner.Score.ToString() + ""
                   , MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.BroadcastMessage).GetArray(stream));
                        ...
                        foreach (var user in Map.Values)
                        {
                            ...
                            MsgSchedules.SendSysMesage("" + Type.ToString() + "City ClanWar has ended. All Players of City has teleported .", MsgServer.MsgMessage.ChatMode.TopLeftSystem, MsgServer.MsgMessage.MsgColor.red);
            ...
            public static uint GetItemReward(CityType typ)
            {
                ...
                {
                    case CityType.Twin:
                        return 722454;//SuperTwinCityClanPack
        ...
        public MsgClanWar()
        {
            ...
            WarCitys.Add(CityType.Twin, new CityWar(CityType.Twin, Database.Server.ServerMaps[1505], 162, 217, 1002, 327, 140));//bahaa
            ...
            WarCitys.Add(CityType.Bird, new CityWar(CityType.Bird, Database.Server.ServerMaps[1507], 95, 113, 1015, 710, 571));
