...
namespace OdysseyServer_Project.Game.MsgTournaments
{
                ...
                if (client.Player.Map == MapID)
                {
                    foreach (var npc in Furnitures.Values)
                    {
                        if (Role.Core.GetDistance(client.Player.X, client.Player.Y, npc.X, npc.Y) <= Role.SobNpc.SeedDistrance)
                        {
                            ...
                            client.Send(stream);
                            if ((Role.SobNpc.StaticMesh)npc.Mesh == Role.SobNpc.StaticMesh.CitywarPole)
    ...
    , MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.BroadcastMessage).GetArray(stream));
        ...
        internal void Save()
        {
            WindowsAPI.IniFile write = new WindowsAPI.IniFile("\\CitywarInfo.ini");
            if (Proces == ProcesType.Dead)
            {
        ...
        internal void Load()
        {
            WindowsAPI.IniFile reader = new WindowsAPI.IniFile("\\CitywarInfo.ini");
