...
namespace LightConquer_Project.Game.MsgTournaments
{
    public class MsgEliteGuildWar
    {
        ...
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Database.Server.ServerMaps[2071].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 821));
        }
        ...
        internal unsafe void Start()
        {
            ...
            using (var rec = new ServerSockets.RecycledPacket())
            {
                ...
                ScoreList.Clear();
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("EliteGuild war has started!", MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
            }
        }
        ...
        internal unsafe void CompleteEndEliteGuildWar()
        {
            SendInvitation = true;
            ShuffleGuildScores();
            Proces = ProcesType.Dead;
...
#else
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the EliteGW with a score of " + Winner.Score.ToString() + ""
                 , MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
#if Encore
                Program.SendGlobalPackets.Enqueue(new MsgServer.MsgMessage("Congratulations to " + Winner.Name + ", they've won the Super Guild War with a score of " + Winner.Score.ToString() + ""
                   , MsgServer.MsgMessage.MsgColor.white, MsgServer.MsgMessage.ChatMode.System).GetArray(stream));
