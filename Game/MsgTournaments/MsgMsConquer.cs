...
namespace OdysseyServer_Project.Game.MsgTournaments
{
    public class MsgMsConquer
    {
        ...
        public MsgMsConquer(ProcesType _Proces)
        {
            ...
            for (TournamentType i = TournamentType.MrsConquer; i < TournamentType.Count; i++)
            {
        ...
        internal MsgServer.MsgUpdate.Flags ObtainAura(TournamentType typ)
        {
            switch (typ)
            {
                case TournamentType.MrsConquer: return MsgServer.MsgUpdate.Flags.TopMsConquer;
            }
        ...
        internal static TournamentType GetMyTournamentType(Client.GameClient client)
        {
            if (Role.Core.IsGirl(client.Player.Body))
                return TournamentType.MrsConquer;
        ...
        public class War
        {
            ...
            public void GetMyReward(Client.GameClient client, ServerSockets.Packet stream)
            {
                if (IsFinish(client))
                {
                    ...
                    string reward = "[MsConquer]" + client.Player.Name + " has received " + RewardConquerPoints + " from class pk.";
                    Program.DiscordEventsAPI.Enqueue($"``{reward}``");
                    Database.ServerDatabase.LoginQueue.Enqueue(reward);
                    LastFlag = aura;
                    Winner = client.Player.UID;
                    MsgSchedules.SendSysMesage("" + client.Player.Name + " Won " + Typ.ToString() + " MsConquer, he received Top " + Typ.ToString() + ", " + RewardConquerPoints.ToString() + " ConquerPoints and  [10] PVE Points!", MsgServer.MsgMessage.ChatMode.TopLeftSystem, MsgServer.MsgMessage.MsgColor.white);
