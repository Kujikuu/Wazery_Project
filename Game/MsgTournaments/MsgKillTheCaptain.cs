    ...
    public class MsgKillTheCaptain : ITournament
    {
        ...
        public void ChooseRandomLeader(KillTheCaptainTeams killTheCaptainTeams)
        {
            ...
            foreach (var player in players)
                if (player.Player.ContainFlag(MsgServer.MsgUpdate.Flags.GodlyShield))
                    player.Player.RemoveFlag(MsgServer.MsgUpdate.Flags.GodlyShield);
            var rndLeader = players[Program.GetRandom.Next(0, players.Count)];
            rndLeader.Player.AddFlag(MsgServer.MsgUpdate.Flags.GodlyShield, 600, true);
            MsgSchedules.SendSysMesage("[KillTheCaptain] Team " + killTheCaptainTeams.ToString() + " your new target is " + rndLeader.Player.Name, MsgServer.MsgMessage.ChatMode.TopLeft, MsgServer.MsgMessage.MsgColor.red);
        }
        private void EndEvent(KillTheCaptainTeams winnerTeam)
        {
            ...
            if (array.Length > 0)
            {
                ...
                if (Winner != null)
                {
                    ...
                    Winner.TeleportCallBack();
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        Winner.Player.RemoveFlag(MsgServer.MsgUpdate.Flags.GodlyShield);
                        var stream = rec.GetStream();
                       
                        Winner.SendSysMesage("You received 12,000 ConquerPoints !", MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.red);
                    }
                    foreach (var user in MapPlayers())
                    {
                        //user.Teleport(440, 368, 1002);//to do
                        user.TeleportCallBack();
                        user.Player.RemoveFlag(MsgServer.MsgUpdate.Flags.GodlyShield);
        ...
        public void CheckUp()
        {
            if (Process == ProcesType.Idle)
            {
                ...
                {
                    Seconds -= 10;
                    MsgSchedules.SendSysMesage("Fight starts in " + Seconds.ToString() + " seconds.", MsgServer.MsgMessage.ChatMode.Center, MsgServer.MsgMessage.MsgColor.red);
                    InfoTimer = DateTime.Now.AddSeconds(10);
                }
            }
            if (Process == ProcesType.Alive)
            {
                if (DateTime.Now > ScoreStamp)
                {
                    ...
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        ...
                        full_red = array.Where(e => e.TeamKillTheCaptain == KillTheCaptainTeams.Red && e.Player.ContainFlag(MsgServer.MsgUpdate.Flags.GodlyShield)).Count() == 0;
                        if (full_red)
                            ChooseRandomLeader(KillTheCaptainTeams.Red);

                        full_blue = array.Where(e => e.TeamKillTheCaptain == KillTheCaptainTeams.Blue && e.Player.ContainFlag(MsgServer.MsgUpdate.Flags.GodlyShield)).Count() == 0;
