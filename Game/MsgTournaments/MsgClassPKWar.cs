        ...
        public class War
        {
            ...
            public void Start(Role.GameMap map)
            {
                if (Proces == ProcesType.Dead)
                {
                    ...
                    {
                        client.Player.MessageBox("", new Action<Client.GameClient>(p => p.Teleport(315, 149, 1002, 0)), null, 60, MsgServer.MsgStaticMessage.Messages.ClassPk);
            ...
            public void GetMyReward(Client.GameClient client,ServerSockets.Packet stream)
            {
                if (IsFinish(client))
                {
                    ...
                    string reward = "[ClassPKWar]" + client.Player.Name + " has received " + RewardConquerPoints + " from class pk.";
