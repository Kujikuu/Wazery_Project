...
namespace OdysseyServer_Project.Database
{
    public static class ShareVIP
    {
        ...
        {
            using (Database.DBActions.Write writer = new DBActions.Write("BindAccounts.txt"))
            {
        ...
        {
            using (Database.DBActions.Read Reader = new DBActions.Read("BindAccounts.txt"))
            {
                if (Reader.Reader())
                {
                ...
                Console.WriteLine(" # Load ["+ Reader.Count + "] Bind Accounts!");
            }
        }
    }
}
